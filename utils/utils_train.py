...
class NoiseDistillationLoss(nn.Module):
    def __init__(self):
        super(NoiseDistillationLoss, self).__init__()

    def forward(self, a, b):
        ce_loss = nn.BCELoss()
        loss = torch.tensor(0.0, device=a.device)
        b = b.to(a.device) if a.device != b.device else b

        for item in range(len(a)):
            loss += ce_loss(a[item].view(a[item].shape[0], -1),
                             b[item].view(b[item].shape[0], -1))
...
class CsfLoss(nn.Module):
    def __init__(self):
        super(CsfLoss, self).__init__()

    def forward(self, a, b):
        loss = torch.tensor(0.0, device=a.device)

        for item in range(len(a)):
            non_zero_indices = (b[item] != 0).nonzero(as_tuple=False)
            loss += torch.mean(a[item][non_zero_indices[:, 0], non_zero_indices[:, 1]])
...
class Revisit_RDLoss(nn.Module):
    ...
    return multi-task loss:  SSOT loss, Reconstruct Loss
    """
    ...
    def forward(self, noised_feature, projected_noised_feature, projected_normal_feature):
        ...
        return (loss_ssot + 0.01 * loss_reconstruct) / 1.01
